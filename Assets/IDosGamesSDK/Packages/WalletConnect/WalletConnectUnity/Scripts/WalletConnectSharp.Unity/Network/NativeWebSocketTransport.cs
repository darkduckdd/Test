...
namespace WalletConnectSharp.Unity.Network
{
	public class NativeWebSocketTransport : MonoBehaviour, ITransport
	{
		...
		private List<string> subscribedTopics = new();
		private Queue<NetworkMessage> _queuedMessages = new();

		public bool Connected => client != null && (client.State == WebSocketState.Open || client.State == WebSocketState.Closing) && opened;
		...
		private async Task _socketOpen()
		{
			...
			TaskCompletionSource<bool> eventCompleted = new(TaskCreationOptions.None);
			...
			_ = nextClient.Connect().ContinueWith(t => HandleError(t.Exception), TaskContinuationOptions.OnlyOnFaulted);
